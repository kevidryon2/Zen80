...
| FLASH A	| FLASH 0-2048 	| 8K 	| Banked Flash memory for permanent storage* |  	|
| FLASH B	| FLASH 0-2048 	| 8K 	| Banked Flash memory for permanent storage* |  	|
| Unused 	| - 	| 7.75K 	|  	|  	|
...
##### $FF66 (VBLEND)
The first 4 bits specify the background layers to be blended, while the last 4 specify the blending modes:
...
##### $FF7B-$FF7C (FLASHBANKB)
Controls the memory bank used by the FLASH memory region B.

##### $FF7D (WAITSCN)
Proceeds to the next scanline when written to.
